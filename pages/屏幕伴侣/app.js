...
function normalizeDiaryBreakTags(value) {
    return String(value ?? "")
        .replace(/&lt;\s*br\s*\/?\s*&gt;/gi, "\n")
        .replace(/<\s*br\s*\/?\s*>/gi, "\n")
        .replace(/[〈＜]\s*br\s*\/?\s*[〉＞]/gi, "\n");
...
function renderDiaryMarkdown(content) {
    ...
    const lines = normalizeDiaryBreakTags(content).replace(/\r\n/g, "\n").split("\n");
    const blocks = [];
    let paragraph = [];
    let listItems = [];
    let codeLines = [];
...
function parseDiaryObservationEntries(content) {
    const text = normalizeDiaryBreakTags(content).replace(/\r\n/g, "\n").trim();
    if (!text) return [];
...
function splitDiaryContent(content) {
    const text = normalizeDiaryBreakTags(content);
    const sections = {
        full: text.trim(),
        observation: "",
        reflection: "",
