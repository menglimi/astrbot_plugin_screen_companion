...
class ScreenCompanionMemoryMixin:
    ...
    def _build_companion_response_guide(self, scene: str, recognition_text: str, custom_prompt: str, context_count: int) -> str:
        """构建同伴响应指南"""
        guide_parts = [
            "回复方式：把这条消息当成聊天里顺手接的一句，不要写成分析报告、战报解说或固定模板。",
            "先说当下最值得提的一点就够了；没有明确建议时，只做自然观察也可以，不必硬凑“夸一句、建议一句、总结一句”。",
            "少用过满的赞美、悬浮夸张和套话，不要总写“漂亮”“稳了”“节奏都在你手里”“妥妥第一梯队”这类用力过猛的话。",
            "如果要提建议，优先只给 1 条和眼前操作直接相关、马上能用上的提醒，尽量用口语顺着说，少用“建议你”“建议最后”这种生硬说法。",
            "不要聊背景音乐、气氛烘托、评分排名、下一局打不打这类和当前判断关系不大的延伸话题。",
            "不要使用 Markdown、加粗、标题、分点、括号旁白或连续感叹号。",
            "把这次回复当成上一条的延续，不要每次都重新播报一遍‘我看到你在做什么’。",
            "如果前面已经提醒过休息，这条默认不要重复催睡。",
        ]

        if scene in ("视频", "阅读"):
            guide_parts.append(
                "当前更适合轻一点的陪伴语气，顺着内容本身说一句观察或理解就够了，别打断沉浸感。"
            )
        else:
            guide_parts.append(
                "更关注用户眼前的操作、风险点和下一步，少讲氛围，多讲真正有用的那个点。"
            )

        if context_count > 0:
            guide_parts.append(
                "参考最近对话保持连贯，只补充新的变化、判断或下一步，不要把上一条又换个说法重讲一遍。"
            )

        if custom_prompt:
            guide_parts.append(
                "用户这轮有明确要求时，优先贴着用户的问题回答，不要被你自己的固定风格带偏。"
            )
